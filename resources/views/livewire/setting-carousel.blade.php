<div>
    ...
    <div class="card">
        <div class="card-body">
            <h4 class="card-title">Setting Carousel</h4>
                      ...
                      <table class="table table-hover">
                        <thead>
                          <tr>
                             ...
                             <th class="text-start">Carousel</th>
                             <th class="text-start">Is Displayed in Homepage?</th>
                             <th class="text-start">Action</th>
                          </tr>
                        </thead>
                        <tbody>
                          @if(($listCarousel!=null)&&(count($listCarousel)>0))
                           @foreach($listCarousel as $val)
                            <tr>
                            <td class="text-center">{{ ($page-1) * $pageSize + $loop->index + 1 }}</td>
                            <td><img src="{{ asset('storage/app/public/carousel/'.$val->carouselImage) }}" wire:click.prevent="openFile('{{$val->carouselImage}}')"></td>
                            <td>
        ...
        @else
                    ...
                    @csrf
                      <div class="form-group row">
                        <label for="flag" class="col-sm-3 col-form-label">Carousel 
                                        @if($action == 'add')
